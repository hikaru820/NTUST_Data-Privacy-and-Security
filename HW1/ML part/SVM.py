...
def train_svm(x_train, x_test, y_train, y_test, label=''):
    ...
    if OUTPUT_MODEL:
        ...
        filename = f"SVMmodels/{label}_SVMmodel_AUC-{best_auc:.4f}_ACC-{best_acc:.4f}_{timestamp}.pkl"
    ...
    if OUTPUT_MODEL:
        ...
        logfile = f"SVMlogs/{label}_SVMresult_{timestamp}.txt"
    ...
    return {'accuracy': accuracy, 'misclass': misclass,
